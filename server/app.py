...
import procesamiento.logic as procesamiento_logic
...
@app.route('/holidays-admin.html')
@require_admin_page
def holidays_admin_page():
...
@app.route('/api/meses-maestria')
@require_login
def get_meses_maestria():
    """Devuelve los meses de maestría-doctorado disponibles ordenados de más reciente a más antiguo."""
    try:
        archivos = [
            f for f in os.listdir(OUTPUT_DIR)
            if f.startswith('data_maestria_doctorado_') and f.endswith('.json')
        ]
        meses = sorted([f[24:-5] for f in archivos], reverse=True)
        return jsonify(meses)
    except Exception:
        logger.exception("Error en /api/meses-maestria")
        return jsonify({'error': 'Error al obtener meses'}), 500
...
def upload():
    ...
    try:
        ...
        mes = procesamiento_logic.procesar(h_path, r_path, OUTPUT_DIR, tipo_horario='oficial')
        logger.info("Mes procesado (oficial): %s por %s", mes, session.get('usuario'))
...
@app.route('/api/upload-maestria', methods=['POST'])
@require_admin
def upload_maestria():
    """
    Recibe los dos Excel para maestría-doctorado, procesa y genera el JSON del mes.
    Form-data: 'horario_maestria' (xlsx), 'registro_maestria' (xlsx)
    """
    _validate_csrf()

    horario  = request.files.get('horario_maestria')
    registro = request.files.get('registro_maestria')

    if not horario or not registro:
        return jsonify({'error': 'Se requieren ambos archivos'}), 400

    for archivo, nombre in [(horario, 'horario_maestria'), (registro, 'registro_maestria')]:
        if not archivo.filename.lower().endswith('.xlsx'):
            return jsonify({'error': f'El archivo {nombre} debe ser .xlsx'}), 400
        if archivo.mimetype and archivo.mimetype not in _XLSX_MIMES:
            return jsonify({'error': f'El tipo del archivo {nombre} no es válido'}), 400
    ...
    try:
        ...
        logger.exception("Error al procesar archivos maestría-doctorado")
        return jsonify({'error': 'Error al procesar los archivos. Verifica el formato de los Excel.'}), 500
...
def delete_mes_maestria(clave):
    ...
    for nombre in [f'data_maestria_doctorado_{clave}.json', f'reporte_asistencia_{clave}.xlsx']:
        ...
        if os.path.exists(path):
            os.remove(path)
            eliminados.append(nombre)
            logger.info("Archivo maestría eliminado: %s por %s", nombre, session.get('usuario'))
